#!/usr/bin/env bash
# Run the high-pT slices 2 (ptj 300-450) and 3 (450-Inf) on a native-amd64 box
# (akasha) at N events each, all 6 variations, with FAST variations first and
# vincia LAST, capped at MAXJOBS concurrent showers so a laptop doesn't OOM/thermal.
# LHE is generated once per slice and cached (reused across the 6 variations).
# Outputs out/slices/mglo_<var>_s<idx>.yoda -- same names the Mac slices use, so
# scp these next to the Mac's s0/s1 and stitch for a uniform-20k 4-slice set.
#
#   ./run_akasha_slices.sh [N=20000] [MAXJOBS=4]
set -uo pipefail
N=${1:-20000}
MAXJOBS=${2:-4}
HERE=$(cd "$(dirname "$0")" && pwd); cd "$HERE"
MG5_IMG=scailfin/madgraph5-amc-nlo:mg5_amc3.5.1
RIV_IMG=hepstore/rivet-pythia:latest
DRUN=(docker run --rm -v "$HERE":/work -w /work)
MGRUN=(docker run --rm -v "$HERE":/work -w /work --entrypoint bash "$MG5_IMG")
mkdir -p out/slices logs

LO=(120 200 300 450); HI=(200 300 450 100000)

# 1. generate (cache) LHE for slices 2 and 3 ---------------------------------
for IDX in 2 3; do
  lo=${LO[$IDX]}; hi=${HI[$IDX]}
  OUT=zjet_mglo_s$IDX
  LHE="$OUT/Events/run_01/unweighted_events.lhe"
  if [ -f "$LHE" ]; then echo "### LHE s$IDX cached, skip gen ###"; continue; fi
  rm -rf "$OUT"
  card=out/slices/_mg5_ak_s$IDX.dat
  { echo "set nb_core 1"; echo "generate p p > l+ l- j"; echo "output $OUT";
    echo "launch $OUT"; echo "set nevents $N"; echo "set iseed $((2000+IDX))";
    echo "set ebeam1 6500"; echo "set ebeam2 6500"; echo "set ptj $lo";
    [ "$hi" -lt 100000 ] && echo "set ptjmax $hi";
    echo "set mmll 60"; echo "set mmllmax 120"; echo "set systematics_program none"; } > "$card"
  echo "### gen LHE slice $IDX (ptj $lo-$hi, N=$N) ###"
  "${MGRUN[@]}" -c "mg5_aMC $card" > logs/gen_s$IDX.log 2>&1
  [ -f "$LHE.gz" ] && gunzip -f "$LHE.gz"
  [ -f "$LHE" ] || { echo "no LHE s$IDX (see logs/gen_s$IDX.log)"; exit 1; }
  echo "### LHE s$IDX done ###"
done

# build plugin + driver once
echo "### build plugin + pythia_rivet ###"
"${DRUN[@]}" "$RIV_IMG" bash -lc '
  export RIVET_ANALYSIS_PATH=/work
  [ -f RivetCMS_ZJET_JETMASS.so ] || rivet-build RivetCMS_ZJET_JETMASS.so CMS_ZJET_JETMASS.cc
  [ -x pythia_rivet ] || g++ gen/pythia_rivet.cc $(pythia8-config --cxxflags --libs) \
      $(rivet-config --cppflags --ldflags --libs) -std=c++17 -o pythia_rivet
' > logs/build.log 2>&1

var_settings() { case "$1" in
  pythia) : ;;
  pythia_vincia)   printf 'PartonShowers:model = 2\n' ;;
  pythia_cr1)      printf 'ColourReconnection:reconnect = on\nColourReconnection:mode = 1\nBeamRemnants:remnantMode = 1\n' ;;
  pythia_cr2)      printf 'ColourReconnection:reconnect = on\nColourReconnection:mode = 2\n' ;;
  pythia_fragsoft) printf 'StringZ:aLund = 0.78\nStringZ:bLund = 1.18\nStringPT:sigma = 0.365\n' ;;
  pythia_fraghard) printf 'StringZ:aLund = 0.58\nStringZ:bLund = 0.78\nStringPT:sigma = 0.305\n' ;;
esac; }

shower_one() {  # IDX name
  local IDX=$1 name=$2
  local LHE="zjet_mglo_s$IDX/Events/run_01/unweighted_events.lhe"
  local cmnd=out/slices/_${name}_ak_s$IDX.cmnd
  { printf 'Beams:frameType = 4\nBeams:LHEF = %s\nPartonLevel:MPI = on\nHadronLevel:all = on\n' "$LHE"
    var_settings "$name"; } > "$cmnd"
  "${DRUN[@]}" "$RIV_IMG" bash -lc "export RIVET_ANALYSIS_PATH=/work; ./pythia_rivet '$cmnd' 'out/slices/mglo_${name}_s$IDX.yoda' $N" \
      > "logs/shw_${name}_s$IDX.log" 2>&1
}

# 2. job list: FAST variations first (both slices), vincia LAST --------------
JOBS=()
for name in pythia pythia_cr1 pythia_cr2 pythia_fragsoft pythia_fraghard; do
  JOBS+=("2:$name" "3:$name")
done
JOBS+=("2:pythia_vincia" "3:pythia_vincia")

echo "### showering ${#JOBS[@]} jobs, $MAXJOBS at a time (vincia last) ###"
running=0
for job in "${JOBS[@]}"; do
  IDX=${job%%:*}; name=${job#*:}
  ( t=$(date +%s); shower_one "$IDX" "$name"
    echo "[$(date +%H:%M:%S)] done $name s$IDX ($(( $(date +%s)-t ))s)" >> logs/akasha_progress.log ) &
  running=$((running+1))
  [ "$running" -ge "$MAXJOBS" ] && { wait -n; running=$((running-1)); }
done
wait
echo "### ALL DONE -> out/slices/mglo_*_s2.yoda out/slices/mglo_*_s3.yoda ###"
ls -la out/slices/mglo_*_s2.yoda out/slices/mglo_*_s3.yoda
